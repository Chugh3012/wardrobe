...
- **Function App EasyAuth v2** — Replaces the previous `ipSecurityRestrictions` (AzureCloud service-tag) approach. EasyAuth v2 is configured in `functions.bicep` via `Microsoft.Web/sites/config@2023-12-01` (`authsettingsV2`) with `unauthenticatedClientAction: 'AllowAnonymous'`. `AllowAnonymous` is required because CORS preflight `OPTIONS` requests carry no Bearer token — `Return401` would block them, causing 403 errors on cross-origin API calls. With `AllowAnonymous`, EasyAuth still validates tokens on authenticated requests and injects `x-ms-client-principal`; unauthenticated requests pass through to the app code which enforces auth via `extractUserId()` + `unauthorizedResponse()`. Only tokens issued by the correct tenant with the correct audience are accepted.
...
- **Base64 client principal validation** — Backend auth middleware decodes and validates the `x-ms-client-principal` base64 header injected by Function App EasyAuth v2. Supports two formats: (a) **SWA format** — extracts `userId` from the top-level `userId` field, and (b) **App Service EasyAuth v2 format** — extracts the Azure AD object ID from the `claims[]` array (checking `objectidentifier` then `nameidentifier` claim types). Validates against safe-character regex and max-length limits. Plain-text header fallback is only accepted when `REQUIRE_AUTH=false` (local development). 25 tests in `authMiddleware.test.ts`.
